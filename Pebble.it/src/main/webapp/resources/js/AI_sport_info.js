...
async function run() {
  ...
  try {
    ...
    const prompt = `나는 특정 스포츠에 대해 매일 새로운 정보를 제공하려고 해
   너는 [수영] 코치고 나는 [수영] [전문가]야
   
   첫 문장에는 오늘 날짜를 포함해서 힘이 날 수 있게 인사하고 
   첫 문장은 <b>태그를 넣어 강조해줘

   나에게 너는 아래의 키워드와 관련된 1가지 운동정보를 제공해줘
   
   모든 키워드에 대한 정보를 제공하지 않아도 되지만 최소 1개에 대한 정보는 제공해줘
   
   응답 내용을 한눈에 볼 수 있도록 소제목을 붙이고 소제목에는 컬러코드는 #DE9631, <h3>태그를 넣고, 끝에 이모지를 넣어줘
   
   응답의 정보 내용은 한줄에 20~50자를 출력하고 줄바꿈하고, 흥미롭고 실용적으로 작성해줘
   줄바꿈 시 꼭 <br>을 넣어서 전체를 태그형태로 출력해줘 html이라는 문구는 출력하지 말아줘
